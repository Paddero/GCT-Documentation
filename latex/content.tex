...
Arjun was responsible for the development of the back-end(Admin Portal) web application and also the API for mobile devices to consume the data from.
Patryk, Trevor, Arjun were responsible for the mobile application and every one of us contributed in the documentation. 
...
\chapter{Introduction and Context}
	...
	\textbf{Methodology}  - This section provides the development methodologies we used to develop this project, including project meetings, collaboration tools used, interaction with the client and weekly meeting with the project supervisor. 
	\textbf{Technology review }: This section provides the research that was carried out when choosing the technology . This section also provides the selection criteria and the particular reason for choosing the specific technology.
	\textbf{System Design, Implementation and Deployment} : This section gives detailed information about actual system, and the way it is functioning. This section also talks about the architecture of a project, Data models, use cases etc.   
	\textbf{System Evaluation :} This section provides evaluates the system based on the following attributes.
	\begin{itemize}
	\item Scalability
	\item Robustness
...
\end{minted}
		...
		We set the start of the google API so we can add the tour points dynamically.\\
\begin{minted}{js}
		...
		\subsection{Presentation Layer}
		 ...
		 \subsection{Ionic Application}
		 			...
		 			This is the landing page of the mobile application. The application is very clean and attractive. It shows the weather and the tabs navigation bar. 
		 				...
		 				\end{center}
		 				When  the user clicks on the take tour button on the previous page, the page above will open. This page displays on the map to the left, the start location of the current selected tour and the next point in the tour. To the right is the information about the current location. This page is dynamic.
		 				\begin{center}    
...
\section{Deployment - Digital Ocean}
	...
	\textbf{The backend(Admin portal) of this system is deployed in digital ocean and can be reached from this link : http://galwaytour.tk/}
...
\textbf{Full content management (Admin portal)} for Galway Civic Trust has been developed and is already hosted on Digital Ocean. Admin Portal provides Galway Civic Trust system admin to perform all the CRUD operations regarding the tours/locations. We have integrated different roles for system admin. Super(root) admin and regular admin. Super admin have privileges to manage other regular system admins, and this was achieved by having a 'Role' field in the database. If system admin forgot their login password, it can be recovered. The password reset link will be sent to his/her email within split seconds and they can go through the process of resetting the password. 
We made sure the API and backend(Admin portal) is fully secured from any external injections or attacks. The API comprises of more than 20 endpoints/routes to perform various types of requests and all the endpoints are fully protected using JWT- JSON web tokens authentication. 


\textbf{The mobile application} has been developed for the consumption of the tours data from an API. Any changes in the datasets is instantly reflected in the consuming mobile devices and synchronisation is very fast and effective. Mobile app provides a complete navigations/directions of a tour. We have integrated with google maps API to provide navigation service. The maps also displays the approximate distance and time to complete a tour and is dynamically changed when use move to different location. Mobile application is making extensive use of google maps API for efficient and reliable navigations. 
...
\chapter{Appendices}
\begin{itemize}
	\item{\textbf{Admin Portal - Backend:  }https://github.com/ultimatecodelab/Galway-Civic-Trust.git}
	\item{\textbf{Ionic Mobile Application: }https://github.com/TrevorDavies/Galway-Civic-Trust.git}
	\item{\textbf{Documentation: }https://github.com/Paddero/GCT-Documentation.git}
\end{itemize}
